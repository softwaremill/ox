...
/** A config that defines how to repeat an operation.
  ...
  * the specified duration after the previous operations has started. If the previous operation takes longer than the interval, the next
  * operation will start immediately after the previous one has finished.
  *
  * It is a special case of [[ScheduledConfig]] with [[ScheduledConfig.sleepMode]] always set to [[SleepMode.StartToStart]] and a

...
trait FlowCompanionIOOps:
  ...
  /** Converts a [[java.io.InputStream]] into a `Flow[Chunk[Bytes]]`.
    ...
    */
  def fromInputStream(is: InputStream, chunkSize: Int = 1024): Flow[Chunk[Byte]] = fromSink: sink =>
    try
      repeatWhile:
  ...
  def fromFile(path: Path, chunkSize: Int = 1024): Flow[Chunk[Byte]] = fromSink: sink =>

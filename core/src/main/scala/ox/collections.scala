...
/** Runs partial function in parallel on each element of `iterable` for which the partial function is defined. If function is not defined
  ...
  *
  * @return
  *   collection of results of applying `pf` to elements of `iterable` for which it is defined. The returned collection is of the same type
  *   as `iterable`
  */
...
/** Runs predicate in parallel on each element of `iterable`. Elements for which predicate returns `true` are returned in the same order as
  * in `iterable`. Elements for which predicate returns `false` are skipped. Using not more than `parallelism` forks concurrently.
  *
  * @tparam I
...
/** Parallelize a foreach operation. Runs the operation on each element of the iterable in parallel. Using not more than `parallelism` forks
  * concurrently.
  *
  * @tparam I

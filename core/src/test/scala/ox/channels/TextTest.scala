...
class TextTest extends AnyFlatSpec with Matchers {
  ...
  it should "split a single chunk of bytes into lines" in supervised {
    ...
    Source.fromValues(chunk).through(text.lines).toList shouldBe List("line1", "line2", "line3")
  }
  ...
  it should "split a single chunk of bytes into lines (beginning with newline)" in supervised {
    val inputText = "\nline1\nline2"
    val chunk = Chunk.fromArray(inputText.getBytes)
    Source.fromValues(chunk).through(text.lines).toList shouldBe List("", "line1", "line2")
  }
  ...
  it should "split a single chunk of bytes into lines (empty array)" in supervised {
    val inputText = ""
    val chunk = Chunk.fromArray(inputText.getBytes)
    Source.fromValues(chunk).through(text.lines).toList shouldBe List.empty
  }
  ...
  it should "split a multiple chunks of bytes into lines" in supervised {
    ...
    Source.fromValues(chunk1, chunk2).through(text.lines).toList shouldBe List("line1-part1,line1-part2", "line2")
  }
  ...
  it should "split a multiple chunks of bytes into lines (multiple newlines)" in supervised {
    ...
    Source.fromValues(chunk1, chunk2, chunk3).through(text.lines).toList shouldBe List("line1-part1,line1-part2", "", "")
  }
  ...
  it should "split a multiple chunks of bytes into lines (multiple empty chunks)" in supervised {
    ...
    Source.fromValues(emptyChunk, emptyChunk, chunk1, emptyChunk).through(text.lines).toList shouldBe List("", "")
  }
}

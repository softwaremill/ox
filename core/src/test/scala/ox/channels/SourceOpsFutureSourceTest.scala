...
class SourceOpsFutureSourceTest extends AnyFlatSpec with Matchers:
  ...
  it should "return the original future failure when future fails" in supervised {
    ...
    Source.fromFutureSource(Future.failed(failure)).receiveOrClosed() shouldBe ChannelClosed.Error(failure)
  }
  ...
  it should "return future's source values" in supervised {
    Source.fromFutureSource(Future.successful(Source.fromValues(1, 2))).toList shouldBe List(1, 2)
  }
end SourceOpsFutureSourceTest

...
class RateLimiterTest extends AnyFlatSpec with Matchers with EitherValues with TryValues with ElapsedTime:
  ...
  it should "drop operation when rate limit is exceeded" in {
    val config = RateLimiterConfig(
      RateLimiterConfig.BlockingPolicy.Drop(),
      RateLimiterConfig.RateLimiterAlgorithm.FixedRate(2, FiniteDuration(1, "second"))
    )
  ...
  it should "block operation when rate limit is exceeded" in {
    val config = RateLimiterConfig(
      RateLimiterConfig.BlockingPolicy.Block(),
      RateLimiterConfig.RateLimiterAlgorithm.FixedRate(2, FiniteDuration(1, "second"))
    )
    val rateLimiter = RateLimiter(config)
    ...
    result3 shouldBe Some(3)
    result4 shouldBe Some(4)
    result5 shouldBe Some(5)
    result6 shouldBe Some(6)
    result7 shouldBe Some(7)
    result8 shouldBe Some(8)
    result9 shouldBe Some(9)
  ...
  it should "drop operation when rate limit is exceeded" in {
    val config = RateLimiterConfig(
      RateLimiterConfig.BlockingPolicy.Drop(),
      RateLimiterConfig.RateLimiterAlgorithm.SlidingWindow(2, FiniteDuration(1, "second"))
    )
    val rateLimiter = RateLimiter(config)
  ...
  behavior of "token bucket RateLimiter"

  it should "drop operation when rate limit is exceeded" in {
    val config = RateLimiterConfig(
      RateLimiterConfig.BlockingPolicy.Drop(),
      RateLimiterConfig.RateLimiterAlgorithm.TokenBucket(2, FiniteDuration(1, "second"))
    )
    val rateLimiter = RateLimiter(config)
  ...
  it should "respect queueing order when blocking concurrently" in {
    ...
    (time2-time1) should be >= 2000L - 10
    (time3-time1) should be >= 3000L - 10
  ...
  behavior of "leaky bucket RateLimiter"

  it should "drop operation when rate limit is exceeded" in {
    val config = RateLimiterConfig(
      RateLimiterConfig.BlockingPolicy.Drop(),
      RateLimiterConfig.RateLimiterAlgorithm.LeakyBucket(2, FiniteDuration(1, "second"))
    )
    val rateLimiter = RateLimiter(config)

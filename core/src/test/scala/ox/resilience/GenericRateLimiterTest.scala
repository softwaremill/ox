...
class GenericRateLimiterTest extends AnyFlatSpec with Matchers with EitherValues with TryValues with ElapsedTime:
  ...
  List(false, true).foreach { fairness =>
    ...
    behavior of s"fixed rate GenericRateLimiter with fairness=$fairness"
    ...
    behavior of s"sliding window GenericRateLimiter with fairness=$fairness"
    ...
    behavior of s"token bucket GenericRateLimiter with fairness=$fairness"
    ...
    behavior of s"leaky bucket GenericRateLimiter with fairness=$fairness"
